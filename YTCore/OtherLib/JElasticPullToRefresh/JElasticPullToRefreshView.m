...
- (void)startLoading
{
    if(_refreshState==JElasticPullToRefreshStateStopped){
        NSLog(@"*********refresh************");

        self.shapeLayer.position = CGPointMake(150, 2);
        [self.loadingView setPullProgress:.95];
        [self layoutSubviews];
        _refreshState = JElasticPullToRefreshStateDragging;
        self.stateChanged = YES;
